...
const StudyRoom = () => {
  ...
  const loadText = useCallback(async (ref: string) => {
    setLoadingText(true);
    try {
      const res = await fetch(`${API_URL}/api/text/${encodeURIComponent(ref)}`);
  ...
  const loadLinks = useCallback(async (ref: string) => {
    try {
      const res = await fetch(`${API_URL}/api/links/${encodeURIComponent(ref)}`);
      if (!res.ok) {
  ...
  const handleOpenPage = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/resolve-ref/${encodeURIComponent(input)}`);
  ...
  const openParallelSource = async (p: ParallelSource) => {
    ...
    setActiveTabId(id);
    try {
      const res = await fetch(`${API_URL}/api/text/${encodeURIComponent(p.ref)}`);
      const data = await res.json();
  ...
  const toggleCommentator = async (c: { en: string; refs: string[] }) => {
    ...
    try {
      const results = await Promise.all(
        c.refs.map((ref) =>
          fetch(`${API_URL}/api/text/${encodeURIComponent(ref)}`).then((r) => r.json())
        )
      );
  ...
  // חסימת התחברות/מנוי לחברותא AI מופעלת אוטומטית רק אחרי שתשלום הוגדר בפועל בשרת -
  // כל עוד לא הוגדר, חברותא AI פתוחה לכולם בלי צורך בהתחברות
  useEffect(() => {
    fetch(`${API_URL}/api/paypal/config`)
      .then((res) => res.json())
      .then((data) => setPaymentsConfigured(!!data.configured))
